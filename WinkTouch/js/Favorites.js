...
export async function allExamPredefinedValues(): ExamPredefinedValue[] {
  ...
  if (
    ...
    examPredefinedValues === null ||
    examPredefinedValues.length === 0
  ) {
...
function getPreviousExamId(
  ...
  visitHistory = visitHistory.filter(
    (visitId: string) =>
      ...
      visitHasStartedExamOfType(visitId, examDefinitionId),
  );
  visitHistory.sort((visitAId: string, visitBId: string) =>
    compareDates(getCachedItem(visitAId).date, getCachedItem(visitBId).date),
  );
...
function getPreviousExamAsFavorite(
  exam: Exam,
  favorite: ExamPredefinedValue,
): ExamPredefinedValue {
  const patientId: string = getPatientId(exam);
  const currentVisitId: string = exam.visitId;
  const lastExamId: ?string = getPreviousExamId(
    patientId,
    currentVisitId,
    favorite.customExamDefinitionId,
  );
  if (lastExamId) {
...
export function getFavorites(exam: Exam): ExamPredefinedValue[] {
  ...
  favorites = favorites.map((examPredefinedValue: ExamPredefinedValue) =>
    examPredefinedValue.predefinedValue === undefined
      ? getPreviousExamAsFavorite(exam, examPredefinedValue)
      : examPredefinedValue,
  );
  return favorites;
}
...
export async function storeFavorite(
  ...
  let predefinedValue: ExamPredefinedValue = {
    id: 'examPredefinedValue',
    version: -1,
    customExamDefinitionId: examDefinitionId,
    predefinedValue: favorite,
    name,
  };
...
export class Star extends PureComponent {
  ...
  render() {
    return [
      <TouchableOpacity
        disabled={this.props.onAddFavorite === undefined}
        onPress={this.activatePopup}
        key="icon"
        testID={this.props.testID ? this.props.testID : 'starIcon'}>
        <Icon
          name="staro"
      ...
      <Modal
        visible={this.state.popupActif}
        transparent={true}
        animationType={'slide'}
        onRequestClose={this.cancelPopup}
        key="popup">
        {this.renderPopup()}
      </Modal>,
    ];
...
export class Close extends PureComponent<CloseProps> {
  render() {
    return (
      <Icon name="close" style={this.props.style} color={selectionFontColor} />
    );
  }
}
...
export class CopyColumn extends PureComponent {
  ...
  render() {
    return (
      <TouchableOpacity onPress={this.props.onPress} testID={this.props.testID}>
        <Icon
          name="doubleright"
        ...
        <View style={styles.verticalFlow}>
          {this.props.favorites &&
            this.props.favorites.map(
              (favorite: ExamPredefinedValue, index: number) => (
                <TouchableOpacity
                  key={favorite.name}
                  onPress={() => {
