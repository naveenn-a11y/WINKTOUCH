...
async function loadConfiguration() {
  ...
  if (localConfiguration===undefined || localConfiguration===null || localConfiguration==='') return;
...
export class ConfigurationScreen extends Component {
  ...
  changeConfiguration(groupName: string, fieldName: string, newValue: any, column: ?string) : void {
    let configuration : Configuration = this.state.configuration;
    configuration[groupName][fieldName]=newValue;
    this.setState({configuration});
    __DEV__ && console.log('saving configuration: '+JSON.stringify(configuration));
