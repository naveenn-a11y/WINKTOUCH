...
export class ExamChartScreen extends Component {
  ...
  generateGraphSeries(exam: Exam) : ChartSeries {
    if (exam.definition.graph===undefined || exam.definition.graph.fields===undefined) return undefined;
    const examHistory : Exam[] = getExamHistory(exam).reverse();
    let labels : string[] = examHistory.map((exam: Exam) => exam?formatMoment(getCachedItem(exam.visitId).date):undefined);
    let series : ChartSeries = exam.definition.graph.fields.map((field: string) => this.generateChartData(field, examHistory));
