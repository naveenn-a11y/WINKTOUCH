...
export async function fetchDocument(documentId: string) {
  try {
    ...
    console.log(
      'Error in fetchDocument for document ' + documentId + ': ' + error,
    );
    alert(
      'Something went wrong trying to get data from the server. You can try again anytime.',
    );
  }
}
...
export async function getRevision(documentId: string): string {
  try {
    ...
    const revision: string = revisionInQuotes.substring(
      1,
      revisionInQuotes.length - 1,
    );
    return revision;
  } catch (error) {
    console.log(
      'Error in getRevision for document ' + documentId + ': ' + error,
    );
    alert(
      'Something went wrong trying to get the newest revision data from the server. You can try again anytime.',
    );
  }
}

export async function fetchViewDocuments(
  view: string,
  startKey: any,
  endKey: any,
) {
  try {
    ...
    alert(
      'Something went wrong trying to get the ' +
        view +
        ' data from the server. You can try again anytime.',
    );
  }
}
...
export async function storeDocument(document: Object) {
  ...
  if (responseJson.ok !== true) {
    ...
    throw new Error(
      'The server could not save your changes because of a ' +
        responseJson.reason +
        '. Please redo your changes.',
    );
  }
